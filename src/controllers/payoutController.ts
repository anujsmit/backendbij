...
import { db } from "../db";
import { users, mistriProfiles, serviceRequests, payouts, appSettings } from "../db/schema";
import {
    and,
    eq,
    desc,
...
export const settleProvider = async (req: Request, res: Response) => {
    try {
        ...
        const [provider] = await db.select().from(users).where(eq(users.id, mistriId)).limit(1);
        if (!provider || provider.role !== "mistri") {
