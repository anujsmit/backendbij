...
export const getUserNotifications = async (req: Request, res: Response) => {
    try {
        ...
        // ✅ Verify user exists based on account type
...
export const createNotification = async (
    ...
    try {
        // ✅ Determine account type by checking all tables
        let accountType: 'user' | 'mistri' | 'admin' = 'user';
        ...
        if (user) {
            ...
            if (mistri) {
                ...
                if (admin) {
                    ...
                    logger.warn(`User ${userId} not found in any account table`);
                    return null;
                }
            }
        }
        ...
        // ✅ Get device token based on account type
        ...
        if (accountType === 'user') {
            ...
            deviceToken = mistri?.deviceToken || null;
        } else if (accountType === 'admin') {
            const admin = await db.query.users.findFirst({
                where: eq(users.id, userId),
            });
