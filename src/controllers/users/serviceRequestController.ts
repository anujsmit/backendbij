...
export const cancelServiceRequest = async (req: Request, res: Response) => {
  try {
    ...
    await createAuditLog({
      ...
      action: 'cancel',
      performedBy: userId,
      performedByRole: (req as any).user?.accountType || 'user',
  ...
  try {
    ...
    if (requestRow.assignedMistriId) {
      const mistriInfo = await db
        ...
        .where(eq(mistriAccounts.id, requestRow.assignedMistriId))
        .limit(1);
    ...
    // ✅ FIXED: Use userAccounts for customer info
    if ((requestRow.status === 'assigned' || requestRow.status === 'completed') && requestRow.assignedMistriId) {
      const customerInfo = await db
        .select({
          name: userAccounts.fullName,
          phone: userAccounts.phoneNumber,
          id: userAccounts.id,
        })
        .from(userAccounts)
        .where(eq(userAccounts.id, requestRow.customerId))
        .limit(1);
...
export const acceptServiceRequest = async (req: Request, res: Response) => {
  try {
    ...
    const customer = await db.query.userAccounts.findFirst({
      where: eq(userAccounts.id, reqRow.customerId),
    });
...
export const getJobHistory = async (req: Request, res: Response) => {
  try {
    ...
    // ✅ FIXED: Use userAccounts for customer name
    const jobs = await db
      .select({
