...
import { db } from "../db";
import { serviceRequests, serviceTypeEnum, locationSourceEnum, serviceRequestStatusEnum, users, mistriProfiles, services, notifications, platformServices, serviceRequestPlatformServices } from "../db/schema";
import { z } from "zod";
import { and, eq, desc, isNull, sql, ne, inArray, gte } from "drizzle-orm";
...
export const acceptServiceRequest = async (req: Request, res: Response) => {
    try {
        ...
        if (customer?.phoneNumber && shouldSendSms) {
            ...
            console.log(`Customer ${reqRow.customerId} has disabled SMS notifications, skipping`);
        }
    ...
    try {
        ...
        if (customer?.phoneNumber && shouldSendCompletionSms) {
            try {
                await sendSms(
                    customer.phoneNumber,
                    `SERVEX: Your ${reqRow.type} service request at ${reqRow.address} has been marked as complete. Thank you for using Mistri!`,
                    "service_completed"
                );
            } catch (smsError) {
                console.error('Failed to send completion SMS:', smsError);
            }
        } else if (!shouldSendCompletionSms) {
            console.log(`Customer ${reqRow.customerId} has disabled SMS notifications, skipping`);
        }
