// backend/src/controllers/admin/serviceSubCategoryItemController.ts
import { Request, Response } from "express";
import { db } from "../../db";
import { 
    serviceItems,        // ✅ Correct table name (Level 3)
  ...
  try {
    ...
    if (!subCategoryId) {
      ...
      const allItems = await db
        ...
        .from(serviceItems)  // ✅ Changed from serviceSubCategoryItems to serviceItems
...
export const createServiceItem = async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db.insert(serviceItems)  // ✅ Changed from serviceSubCategoryItems to serviceItems
      .values({
...
export const updateServiceItem = async (req: Request, res: Response) => {
  try {
    ...
    const [updated] = await db
      .update(serviceItems)  // ✅ Changed from serviceSubCategoryItems to serviceItems
      .set(updateData)
      .where(eq(serviceItems.id, id))
      .returning();
...
export const deleteServiceItem = async (req: Request, res: Response) => {
  try {
    ...
    await db.delete(serviceItems).where(eq(serviceItems.id, id));  // ✅ Changed from serviceSubCategoryItems to serviceItems
